...
public class ConsoleAPI { [StructLayout(LayoutKind.Sequential)] public struct COORD { public short X; public short Y; }; 
...
function IsNumeric { param ([string]$str) return $str -match '^[0-9]+$' }; 

function ParseBounds { param ([string]$rangeStr) 
	if (-not $rangeStr) { return 0, 0xFFF; }; 
...
function PrintAttributes { param ( [string]$prefix, [UInt16]$attributes ) 
...
function PrintInfo { param ( [string]$prefix, [ConsoleAPI+CONSOLE_SCREEN_BUFFER_INFO]$csbi, [string]$idx, [IntPtr]$potentialHandle ) 
...
function ProcessRange { param ( [string]$prefix, [string]$rangeStr, [ref]$idx ) 
	$startBound, $endBound = ParseBounds $rangeStr; 
	for ($i = $startBound; $i -le $endBound; $i++) {
		$potentialHandle = [IntPtr]$i; $csbi = [ConsoleAPI+CONSOLE_SCREEN_BUFFER_INFO]::new(); 
		if ([ConsoleAPI]::GetConsoleScreenBufferInfo($potentialHandle, [ref]$csbi)) { 
			$idx.Value++; PrintInfo $prefix $csbi $idx.Value $potentialHandle; 
		}; 
	}; 
}; 
$prefix = "console.screenbuffers"; $idx = -1; $argIndex = 1; 
if ($args[0] -eq "/?" -or $args[0] -eq "/h") { "Usage: yourscript.ps1 optional <prefix> optional [range1] [range2] [rangeN] ..."; return; }; 
if ( -gt 0 -and -not (IsNumeric $args[0]) -and ($args[0] -notmatch '-')) {$prefix = $args[0]; $argIndex++; }; 
if ($args.Count -ge 1 -and $args[0] -eq "active") { 
$prefix = "console.screenbuffers"; }; 
$csbi = [ConsoleAPI+CONSOLE_SCREEN_BUFFER_INFO]::new(); 
$activeScreenBufferHandle = [ConsoleAPI]::GetStdHandle(-11); 
if ([ConsoleAPI]::GetConsoleScreenBufferInfo($activeScreenBufferHandle, [ref]$csbi)) { 	PrintInfo $prefix $csbi "active" $activeScreenBufferHandle; }; 
if ($args.Count -ge 1 -and $args[0] -eq "active") { return; }; 
if ($argIndex -lt $args.Count) { 
	for (; $argIndex -lt $args.Count; $argIndex++) { ProcessRange $prefix $args[$argIndex] ([ref]$idx); }; 
} else { ProcessRange $prefix "" ([ref]$idx); }; 
if ($idx -ne -1) { "$prefix.ubound=$idx"; };
